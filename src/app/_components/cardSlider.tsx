...
function Card({ type }: { type: 'rock' | 'scissors' | 'paper' }) {
    return <button className="flex-shrink-0 w-[100px] h-[150px]  md:w-[210px]  md:h-[300px] bg-[#2D163F] rounded-md shadow-xl"><p className="text-xl md:text-[56px]">{type}</p></button>
}
...
export default function CardSlider({ user, isOwner }: { user: User, isOwner: boolean }) {
    ...
    const types: ('rock' | 'scissors' | 'paper')[] = ['rock', 'paper', 'scissors']
    return <div className={`h-[calc(((50vw*sqrt(3))/3)+330px)] w-[calc(50%-2px)] md:w-[calc(50%-20px)] flex relative items-center overflowd-hidden transition-opacity ${visible ? 'opacity-100' : 'opacity-0'}`} style={{ scrollbarWidth: 'none', maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}>
        <p className={`absolute text-center max-w-[500px] text-2xl md:text-6xl h-fit z-10 top-16 sm:top-24 inset-x-0 mx-auto origin-center ${isOwner ? 'rotate-[30deg]' : 'rotate-[-30deg]'}`}>{user.name} Threw:</p>
        <div className={`flex items-center relative  ${isOwner ? 'translate-x-[-20%]' : 'translate-x-[-20%]'}`}>
